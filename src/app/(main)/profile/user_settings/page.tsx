...
import { changeUserData, changeUserEmail } from '@/components/server/changeUserProfile';
...
>("idle");
  ...
  const {register: registerEmail, handleSubmit: handleSubmitEmail, formState: {errors: emailErrors}, setValue: setEmailValue, trigger: triggerEmail, reset: resetEmail} = useForm<changeEmailData>({
  ...
  const isDisabled =
  emailStatus !== "available" || !isVerifyed;
  ...
  return (
    <div className="p-4 bg-cyan-950 text-amber-100 rounded-t-xl">
      ...
      <form onSubmit={handleSubmitEmail(onSubmitEmail)} className="flex flex-col gap-y-2 md:max-w-2/3 md:mx-auto">
...
/>
    ...
    <p className="h-10 text-red-500">
      ⚠️ Пошта <span className='text-orange-300 font-bold'>{userEmail}</span> не підтверджена!
    </p>
  )}
